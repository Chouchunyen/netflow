...
art_cost = {
    ("add","Artificial","i1"): 10,
    ("add","Artificial","i2"): 10,
    ("add","Artificial","i3"): 10,
    ("cut","Artificial","i1"): 10,
    ("cut","Artificial","i2"): 10,
    ("cut","Artificial","i3"): 10,
}
...
for h in lacks:
    for i,j in inc_arcs:
        inc_cost[h,i,j] =  m.addVar(lb=8, ub=10, vtype=GRB.INTEGER, name='inc_cost_%s_%s_%s' % (h, i, j))
...
for h in lacks:
    for i,j in inc_arcs:
        m.addGenConstrPWL(flow[h,i,j], inc_cost[h,i,j], [0, 6000,  6001,10000], [10, 10, 8, 8])
